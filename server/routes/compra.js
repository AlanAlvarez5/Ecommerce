...
router.get('/select-detail/:id', async(req, res) => {
    ...
    try {
        ...
        let detalles = await db.query(`SELECT detalle.producto_id, detalle.cantidad, producto.nombre, producto.marca, producto.precio,producto.precio*detalle.cantidad AS total, producto.imagen FROM detalle JOIN producto ON producto.id = detalle.producto_id and detalle.compra_id = '${id}'`);
        res.json(detalles);
    } catch (error) {
        return res.status(400).json({
            mensaje: 'Query Error',
